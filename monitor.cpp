...
void Monitor::push_priority(int id, int priority){
  ...
  j = mod_m(this->back - 1, this->size);

  for (int i = 0; i < this->actual_size; i++) {
    if (this->buffor[j].priority >= priority ) {
      this->buffor[((j + 1) % this->size)].id = id;
      this->buffor[((j + 1) % this->size)].priority = priority;
      this->actual_size += 1;
      this->back = (this->back + 1) % this->size;
      return;
    }
    this->buffor[((j + 1) % this->size)].id = this->buffor[j].id;
    this->buffor[((j + 1) % this->size)].priority = this->buffor[j].priority;
    j = mod_m(j - 1, this->size);
  }
  this->buffor[((j + 1) % this->size)].id = id;
  this->buffor[((j + 1) % this->size)].priority = priority;
  this->actual_size += 1;
